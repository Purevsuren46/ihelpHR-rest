...
exports.getCvFollows = asyncHandler(async (req, res, next) => {
    ...
    const follows = await Follow.find(req.query, select).sort(sort).skip(pagination.start - 1).limit(limit).populate({path: 'followUser', select: 'firstName lastName profile occupation organization'}).populate({path: 'createUser', select: 'organization'})
